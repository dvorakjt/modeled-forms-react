...
describe('useOmittableFormElement', () => {
  ...
  test('It returns a boolean value that is updated when setOmitFormElement() is called.', async () => {
    ...
    const { result : useOmittableFormElementResult } = renderHook(() => useOmittableFormElement('fieldA'));
    ...
    await waitFor(() => expect(useOmittableFormElementResult.current.omitFormElement).toBe(false));
  });
});