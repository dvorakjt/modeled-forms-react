...
describe('useDualField()', () => {
  test('An error is thrown if the field name does not exist in the template.', () => {
    ...
    const { errorDetected } = renderPossiblyErrantHook(() =>
      useDualField('someNonExistentField'),
    );
  ...
  test('It returns usePrimaryField().', () => {
    ...
    const { useDualField } = useRootFormResult.current;
    const { result: useDualFieldResult } = renderHook(() =>
      useDualField('dualField'),
    );
  ...
  test('usePrimaryField() returns useField, passing in the field created from the template.', async () => {
    ...
    const { usePrimaryField } = useDualFieldResult.current;
    const { result: usePrimaryFieldResult } = renderHook(() =>
      usePrimaryField(),
    );
    const { value, validity, messages, modified, updateValue, visit, reset } =
      usePrimaryFieldResult.current;
    ...
    await waitFor(() =>
      expect(usePrimaryFieldResult.current.visited).toBe(Visited.YES),
    );
  ...
  test('useSecondaryField() returns useField, passing in the field created from the template.', async () => {
    ...
    const { useSecondaryField } = useDualFieldResult.current;
    const { result: useSecondaryFieldResult } = renderHook(() =>
      useSecondaryField(),
    );
    const { value, validity, messages, modified, updateValue, visit, reset } =
      useSecondaryFieldResult.current;
    ...
    await waitFor(() =>
      expect(useSecondaryFieldResult.current.visited).toBe(Visited.YES),
    );
  ...
  test('reset() resets the field.', async () => {
    ...
    const { result: useSwitchToSecondaryFieldResult } = renderHook(() =>
      useSwitchToSecondaryField(),
    );
    ...
    await waitFor(() =>
      expect(usePrimaryFieldResult.current.value).toBe('updated primary field'),
    );

    useSecondaryFieldResult.current.updateValue('updated secondary field');

    await waitFor(() =>
      expect(useSecondaryFieldResult.current.value).toBe(
        'updated secondary field',
      ),
    );
    ...
    await waitFor(() =>
      expect(useSwitchToSecondaryFieldResult.current.useSecondaryField).toBe(
        true,
      ),
    );
    ...
    await waitFor(() =>
      expect(usePrimaryFieldResult.current.value).toBe('primary'),
    );
