...
describe('useNestedForm', () => {
  test('It throws an error if the field name is not found in the template.', () => {
    ...
    const { errorDetected } = renderPossiblyErrantHook(() =>
      useNestedForm('nonExistentNestedForm'),
    );
  ...
  test('It returns useForm, passing in the nestedForm created by the template parser.', () => {
    ...
    const { result: useNestedFormResult } = renderHook(() =>
      useNestedForm('someNestedForm'),
    );
    ...
    expect(typeof useNestedFormResult.current.useOmittableFormElement).toBe(
      'function',
    );
    expect(typeof useNestedFormResult.current.useExtractedValue).toBe(
      'function',
    );
    expect(typeof useNestedFormResult.current.useFirstNonValidFormElement).toBe(
      'function',
    );
    expect(typeof useNestedFormResult.current.useDualField).toBe('function');
  });
