...
describe('AsyncAdapter', () => {
  ...
  const syncAdapterFn : SyncAdapterFn<typeof fields, string> = ({testField}) => {
  ...
  test('stream emits a new value when the aggregator emits a new value and the adapterFn emits a new value.', () => {
    const multiFieldAggregator = new MultiFieldAggregatorStub<typeof fields>(subscriptionManager);
...
function triggerableAsyncAdapter<Fields extends FormElementMap, V>(subject : Subject<void>, adapterFn : SyncAdapterFn<Fields, V>) {
