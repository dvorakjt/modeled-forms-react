...
describe('UserFacingMultiInputValidatedNestedForm', () => {
  test('userFacingFields returns the base nested form\'s userFacingFields.', () => {
    ...
    const multiInputValidatedFormElements = container.services.MultiInputValidatedFormElementFactory.createUserAndFinalizerFacingMultiInputValidatedFormElement(
      baseNestedForm
    );

    const userFacingMultiInputValidatedNestedForm = multiInputValidatedFormElements[0] as UserFacingMultiInputValidatedNestedForm;

    expect(userFacingMultiInputValidatedNestedForm.userFacingFields).toBe(baseNestedForm.userFacingFields);
  });
  ...
  test('extractedValues returns the base nested form\'s extractedValues.', () => {
    ...
    expect(userFacingMultiInputValidatedNestedForm.extractedValues).toBe(baseNestedForm.extractedValues);
  });
  ...
  test('state returns the base nested form\'s messages.', () => {
    ...
    expect(userFacingMultiInputValidatedNestedForm.state.messages).toStrictEqual([
      {
  ...
  test('state returns the minimum of the base nested form\'s validity and the MultiInputValidatorReducer\'s validity.', () => {
    ...
    expect(userFacingMultiInputValidatedNestedForm.state.validity).toBe(Validity.INVALID);

    (baseNestedForm.userFacingFields.fieldA as AbstractField).setValue('some value');

    expect(userFacingMultiInputValidatedNestedForm.state.validity).toBe(Validity.VALID_FINALIZABLE);
    ...
    const mockMultiInputValidatorBaseFn : SyncValidator<AggregatedStateChanges> = ({nestedForm, someField}) => {
      return {
        isValid : false
      }
    }

    const multiInputValidator = container.services.MultiInputValidatorFactory.createSyncMultiInputValidator(mockMultiInputValidatorBaseFn, fields);
  ...
  test('state returns the base nested form\'s visited property.', () => {
    ...
    expect(userFacingMultiInputValidatedNestedForm.state.visited).toBe(Visited.PARTIALLY);
  });
  ...
  test('state returns the base nested form\'s modified property.', () => {
    ...
    expect(userFacingMultiInputValidatedNestedForm.state.modified).toBe(Modified.PARTIALLY);
  });
  ...
  test('firstNonValidFormElement returns the base nested form\'s firstNonValidFormElement.', () => {
    ...
    expect(userFacingMultiInputValidatedNestedForm.firstNonValidFormElement).toBe('fieldB');
  });
  ...
  test('firstNonValidFormElementChanges returns the base nested form\'s firstNonValidFormElementChanges.', () => {
    ...
    expect(userFacingMultiInputValidatedNestedForm.firstNonValidFormElementChanges).toBe(baseNestedForm.firstNonValidFormElementChanges);
  });
