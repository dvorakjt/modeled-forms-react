...
describe('FinalizerFacingMultiInpuValidatedFormElement', () => {
  test("state returns the base form element's value.", () => {
    ...
    const finalizerFacingMultiInputValidatedFormElement =
      multiInputValidatedFormElements[1];

    expect(finalizerFacingMultiInputValidatedFormElement.state.value).toBe(
      expectedValue,
    );
  });
  ...
  test("state returns the base form element's messages.", () => {
    ...
    expect(
      finalizerFacingMultiInputValidatedFormElement.state.messages,
  ...
  test("state returns the base form element's visited.", () => {
    ...
    expect(finalizerFacingMultiInputValidatedFormElement.state.visited).toBe(
      Visited.YES,
    );
  });
  ...
  test("state returns the base form element's modified.", () => {
    ...
    expect(finalizerFacingMultiInputValidatedFormElement.state.modified).toBe(
      Modified.YES,
    );
  });
  ...
  test("state returns the base form element's useSecondaryField if the base form element is an instance of AbstractDualField.", () => {
    ...
    expect(
      finalizerFacingMultiInputValidatedFormElement.state.useSecondaryField,
    ).toBe(true);
  });
  ...
  test("state returns the minimum of the base form element's validity and any attached validator's validitiy.", () => {
    ...
    expect(finalizerFacingMultiInputValidatedFormElement.state.validity).toBe(
      Validity.INVALID,
    );
    ...
    finalizerFacingMultiInputValidatedFormElement.addValidator(
      multiInputValidator,
    );
