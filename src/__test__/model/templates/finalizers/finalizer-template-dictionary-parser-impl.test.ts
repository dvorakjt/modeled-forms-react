...
describe('FinalizerTemplateDictionaryParserImpl', () => {
  const finalizerTemplateDictionaryParser =
    container.services.FinalizerTemplateDictionaryParser;
  ...
  test('A finalizer entry with a syncFinalizerFn property should have a corresponding SyncFinalizer created in finalizers.', () => {
    ...
    const finalizerManager = finalizerTemplateDictionaryParser.parseTemplate(
      template,
      finalizerFacingFields,
    ) as FinalizerManagerImpl;

    expect(finalizerManager._finalizerMap.fieldAToUpper).toBeInstanceOf(
      SyncFinalizer,
    );
  });
  ...
  test('A finalizer entry with an asyncFinalizerFn property should have a corresponding AsyncFinalizer created in finalizer.', () => {
    ...
    expect(finalizerManager._finalizerMap.fieldAToUpperAsync).toBeInstanceOf(
      AsyncFinalizer,
    );
  });
  ...
  test('A field passed in as an argument to a sync finalizer SHOULD receive a default finalizer if preserveOriginalFields is true.', () => {
    ...
    expect(finalizerManager._finalizerMap.fieldA).toBeInstanceOf(
      DefaultFinalizer,
    );
  });
  ...
  test('Fields not included in finalizer template dictionary should receive default finalizers.', () => {
    ...
    expect(finalizerManager._finalizerMap.fieldB).toBeInstanceOf(
      DefaultFinalizer,
    );
  });
});
