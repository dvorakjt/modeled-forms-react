...
describe('ExtractedValuesTemplateParserImpl', () => {
  ...
  test('It returns an empty ExtractedValueDictionary if both syncExtractedValues and asyncExtractedValues are undefined.', () => {
    ...
    expect(extractedValuesTemplateParser.parseTemplate(extractedValuesTemplate, fields)).toStrictEqual({});
  });
    ...
    const extractedValuesTemplate : ExtractedValuesTemplate = {
      syncExtractedValues : {
        favoriteColorCompliment : ({ favoriteColor }) => {
          ...
          return complimentaryColors[favoriteColor.value as keyof typeof complimentaryColors];
        }
      }
    };

    const extractedValues = extractedValuesTemplateParser.parseTemplate(extractedValuesTemplate, fields);
  ...
  test('It returns an ExtractedValueDictionary with the expected keys when asyncExtractedValues contains keys.', () => {
    const restaurantMenu = [
      'grilled tofu', 'impossible burger', 'vegetarian hoagie'
    ]
    ...
    expect(extractedValues.restaurantHasFavoriteFood).toBeInstanceOf(AsyncAdapter);
  });
});