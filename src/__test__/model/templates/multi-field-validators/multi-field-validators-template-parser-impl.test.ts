...
describe('MultiFieldValidatorsTemplateParserImpl', () => {
  test('A template containing an async field will produce async MultiInpuValidators.', () => {
    ...
    const parsed = container.services.MultiFieldValidatorsTemplateParser.parseTemplate(template, fields);
    const userFacingFields = parsed[0];
    expect(userFacingFields.fieldA).toBeInstanceOf(UserFacingMultiInputValidatedField);
    expect(userFacingFields.fieldB).toBeInstanceOf(UserFacingMultiInputValidatedField);
    expect(userFacingFields.fieldC).toBeInstanceOf(UserFacingMultiInputValidatedField);
    expect(userFacingFields.fieldD).toBeInstanceOf(UserFacingMultiInputValidatedField);
  });
});