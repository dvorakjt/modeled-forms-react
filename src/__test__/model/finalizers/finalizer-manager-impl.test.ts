...
describe('FinalizerManagerImpl', () => {
  ...
  beforeEach(() => {
    ...
    const finalizerValidityReducer =
      reducerFactory.createFinalizerValidityReducer();
  ...
  test("When a finalizer emits a FinalizerState with a defined value, the FinalizerManager's value property is updated.", () => {
    ...
    finalizerDictionary.fieldA.stream.next({
      value: 'new value',
  ...
  test('If one or more finalizers throws an error, the messages property contains the global finalizerError message.', () => {
    ...
    const finalizerErrorMessage =
      configLoader.config.globalMessages.finalizerError;
