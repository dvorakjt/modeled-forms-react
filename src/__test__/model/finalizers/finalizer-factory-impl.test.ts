...
describe('FinalizerFactoryImpl', () => {
  ...
  beforeEach(() => {
    finalizerFactory = new FinalizerFactoryImpl(container.services.AggregatorFactory, container.services.FinalizerValidityTranslator);
  });

  test('createSyncFinalizer() returns a SyncFinalizer when called.', () => {
    const syncFinalizerFn : SyncFinalizerFn = ({ fieldA }) => ({
      value : fieldA.validity === Validity.VALID_FINALIZABLE ? fieldA.value.toUpperCase() : undefined,
      finalizerValidity : fieldA.validity ===  Validity.VALID_FINALIZABLE ? FinalizerValidity.VALID_FINALIZED : FinalizerValidity.FIELD_INVALID,
      modified : fieldA.modified,
      visited : fieldA.visited
    });
    const fieldA = new MockField('', Validity.VALID_FINALIZABLE);
    const fields = { fieldA };
    const finalizer = finalizerFactory.createSyncFinalizer(syncFinalizerFn, fields);
  ...
  test('createAsyncFinalizer() returns an AsyncFinalizer when called.', () => {
    ...
    const fieldA = new MockField('', Validity.VALID_FINALIZABLE);
    const fields = { fieldA };
    const finalizer = finalizerFactory.createAsyncFinalizer(asyncFinalizerFn, fields);
