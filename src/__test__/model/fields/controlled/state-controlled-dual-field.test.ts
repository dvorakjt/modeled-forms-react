...
describe('StateControlledDualField', () => {
  ...
  const simpleControlFn : SyncDualFieldStateControlFn = ({ fieldA }) => {
    return ({
      primaryFieldState : {
        value : fieldA.validity === Validity.VALID_FINALIZABLE ? fieldA.value.toUpperCase() : '',
  ...
  test('When it receives an error from the adapter, if useSecondaryField is false, it sets the primaryField\'s state to an expected error state.', () => {
    ...
    const fields = { fieldA, fieldB };
    const controlledField = controlledFieldFactory.createStateControlledFieldWithSyncAdapter(fieldB, errantControlFn, fields);
  ...
  test('getting the field\'s omit property returns the internal field\'s omit property.', () => {
    ...
    const fieldA = baseFieldFactory.createField('', false, [], []);
    const fieldB = baseFieldFactory.createDualField('', '', initialOmit, [], []);
    const fields = { fieldA, fieldB };
    const controlledField = controlledFieldFactory.createStateControlledFieldWithSyncAdapter(fieldB, simpleControlFn, fields);
  ...
  test('getting the field\'s primaryField property returns the inner field\'s primary field property.', () => {
    ...
    const fields = { fieldA, fieldB };
    const controlledField = controlledFieldFactory.createStateControlledFieldWithSyncAdapter(fieldB, simpleControlFn, fields);
    expect((controlledField as StateControlledDualField).primaryField).toBe(fieldB.primaryField);
  });
  ...
  test('getting the field\'s secondaryField property returns the inner field\'s primary field property.', () => {
    ...
    const fields = { fieldA, fieldB };
    const controlledField = controlledFieldFactory.createStateControlledFieldWithSyncAdapter(fieldB, simpleControlFn, fields);
    expect((controlledField as StateControlledDualField).secondaryField).toBe(fieldB.secondaryField);
  });
  ...
  test('getting the field\'s useSecondaryField property returns the internal field\'s useSecondaryField property.', () => {
    ...
    expect((controlledField as StateControlledDualField).useSecondaryField).toBe(false);

    fieldB.useSecondaryField = true;
    expect((controlledField as StateControlledDualField).useSecondaryField).toBe(true);
  });
