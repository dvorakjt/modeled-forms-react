...
describe('Field', () => {
  ...
  test('The value property of its initial state is equal to the default value passed into its constructor.', () => {
    const validatorSuite =
      validatorSuiteFactory.createSingleInputValidatorSuite<string>([], []);
  ...
  test('The validity property of its initial state is equal to the syncResult of the validator suite passed in.', () => {
    const mockSyncValidator: SyncValidator<string> = () => ({
      isValid: false,
      message: 'test message',
    });
    const validatorSuite =
      validatorSuiteFactory.createSingleInputValidatorSuite(
        [mockSyncValidator],
        [],
      );
  ...
  test('The messages property of its initial state is equal to the aggregated messages of the syncResult property of its validator suite.', () => {
    ...
    const mockValidatorA: SyncValidator<string> = () => ({
      isValid: true,
      message: expectedMessages[0].text,
    });
    const mockValidatorB: SyncValidator<string> = () => ({
      isValid: false,
      message: expectedMessages[1].text,
    });
    const validatorSuite =
      validatorSuiteFactory.createSingleInputValidatorSuite(
        [mockValidatorA, mockValidatorB],
        [],
      );
    const field = new Field(validatorSuite, '', false);
  ...
  test('If an async validator suite is passed into the constructor, it validates the default value.', () => {
    ...
    const mockValidator: SyncValidator<string> = (value: string) => ({
      isValid: value.length > 0,
      message: 'test async validator message',
    });
    const asyncValidator = createTriggerableAsyncValidator(
      trigger,
      mockValidator,
    );
    const validatorSuite =
  ...
  test('When setValue() is called, an existing validator subscription will be unsubscribed from.', () => {
    ...
    const currentSubscription =
      field._validatorSuiteSubscription as Subscription;
  ...
  test('Calling setValue() sets the value, validity and messages to the syncResult of its validator suite.', () => {
    ...
    expect(actualValueValidityMessages).toStrictEqual(
      expectedValueValidityMessages,
    );
  });
