...
describe('MultiFieldAggregator', () => {
  ...
  test('It updates hasOmittedFields when field.omit is true.', () => {
    let updateCount = 0;
    const adapter = ({ fieldA, hasOmittedFields } : AggregatedStateChanges<any>)  => {
      if(++updateCount === 2) {
  ...
  test('It receives the correct overallValidity and hasOmittedFields at the time of subscription', () => {
    ...
    const adapter = ({fieldA, fieldB, hasOmittedFields, overallValidity} : AggregatedStateChanges<any>) => {
