...
import { ControlledFieldFactory, ControlledFieldFactoryKey } from "../../../fields/controlled/controlled-field-factory.interface";
...
import { ControlledFieldTemplateParser, ControlledFieldTemplateParserKey, ControlledFieldTemplateParserKeyType } from "./controlled-field-template-parser.interface";
...
class ControlledFieldTemplateParserImpl implements ControlledFieldTemplateParser {
  ...
  private getControlFnType(template : ControlledFieldTemplateVariations) : ControlType {
    ...
    for(const controlType in controlTypes) {
      if(controlType in template && template[controlType as keyof typeof template]) {
        if(controlFnType) {
          throw new ControlledFieldTemplateParsingError('ControlledFieldTemplateParser received template containing multiple control functions. Please include only one control function type.')
        }
        controlFnType = controlType as ControlType;
      }
    }
    if(!controlFnType) throw new ControlledFieldTemplateParsingError('The template passed to ControlledFieldTemplateParser lacked a control function.');
  
    return controlFnType;
  }
}

const ControlledFieldTemplateParserService = autowire<ControlledFieldTemplateParserKeyType, ControlledFieldTemplateParser, ControlledFieldTemplateParserImpl>(
...
export { ControlledFieldTemplateParserImpl, ControlledFieldTemplateParserService };