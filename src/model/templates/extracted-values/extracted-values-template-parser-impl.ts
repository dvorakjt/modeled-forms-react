...
import {
  ...
  ExtractedValuesTemplateParserKeyType,
} from './extracted-values-template-parser.interface';
import { ExtractedValuesTemplate } from './extracted-values-template.interface';

class ExtractedValuesTemplateParserImpl
  implements ExtractedValuesTemplateParser
{
  ...
  parseTemplate(
    ...
    if (template.syncExtractedValues) {
      for (const key in template.syncExtractedValues) {
        extractedValuesDictionary[key] =
          this._adapterFactory.createSyncAdapterFromFnWithFields(
            template.syncExtractedValues[key],
            fields,
          );
      }
    }
    if (template.asyncExtractedValues) {
      for (const key in template.asyncExtractedValues) {
        extractedValuesDictionary[key] =
          this._adapterFactory.createAsyncAdapterFromFnWithFields(
            template.asyncExtractedValues[key],
            fields,
          );
      }
    }
...
const ExtractedValuesTemplateParserService = autowire<
  ExtractedValuesTemplateParserKeyType,
  ExtractedValuesTemplateParser,
  ExtractedValuesTemplateParserImpl
>(ExtractedValuesTemplateParserImpl, ExtractedValuesTemplateParserKey, [
  AdapterFactoryKey,
]);

export {
  ExtractedValuesTemplateParserImpl,
  ExtractedValuesTemplateParserService,
};
