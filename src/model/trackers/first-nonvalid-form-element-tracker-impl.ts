...
export class FirstNonValidFormElementTrackerImpl implements FirstNonValidFormElementTracker {
  ...
  constructor(nonValidFormElementHeap : InsertionOrderHeap) {
    ...
    this.firstNonValidFormElement = new BehaviorSubject(this.#nonValidFormElementHeap.topValue);
  }
  ...
  trackFormElementValidity(formElementKey : string, formElement: StatefulFormElement<any>): void {
    ...
    formElement.stateChanges.subscribe(({validity}) => {
      ...
      this.firstNonValidFormElement.next(this.#nonValidFormElementHeap.topValue);
    });
  }
}