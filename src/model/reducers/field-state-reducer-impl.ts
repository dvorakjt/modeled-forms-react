...
type TallyKey = 'errantFields' | 'invalidFields' | 'pendingFields' | 'validUnfinalizableFields' | 'omittedFields';
...
export class FieldValidityReducerImpl implements FieldStateReducer {
  ...
  updateTallies(fieldName: string, state : FieldState) {
    ...
    this.updateTally(fieldName, validity, Validity.VALID_UNFINALIZABLE, 'validUnfinalizableFields');
  ...
  updateTally(
    ...
    tallyName : 'errantFields' | 'invalidFields' | 'pendingFields' | 'validUnfinalizableFields'
  ) {
    if (actualValidity === expectedValidity) this.#tallies[tallyName]
